...
const blankForm = { title: "", referenceNumber: "", documentDate: "2026-07-31", category: "Surat", archiveYear: 2026, description: "" };
...
export default function DocumentArchive() {
  ...
  async function loadDocuments() {
    ...
    } catch { setMessage("Penyimpanan arsip sedang disiapkan. Coba muat ulang beberapa saat lagi."); }
  ...
  async function submit(event: FormEvent) {
    ...
    if (!file) { setMessage("Pilih file yang ingin diarsipkan."); return; }
    ...
    try {
      ...
      setDocuments((old) => [data.document, ...old]); setForm(blankForm); setFile(null); setShowForm(false); setMessage("Dokumen berhasil diarsipkan.");
    } catch (error) { setMessage(error instanceof Error ? error.message : "Gagal mengarsipkan dokumen."); }
    finally { setSaving(false); }
  }

  const totalSize = documents.reduce((sum, d) => sum + d.sizeBytes, 0);
  const thisYear = documents.filter((d) => d.archiveYear === 2026).length;
  ...
  return <div className="page mail-page archive-page">
    ...
    <section className="mail-stats archive-stats">
      ...
      <article><span>Arsip Tahun 2026</span><strong>{thisYear}</strong><small>Dokumen tahun berjalan</small></article>
    ...
    <section className="panel mail-table-panel"><div className="mail-toolbar">
      <label className="mail-search"><span>⌕</span><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Cari judul, referensi, nama file, atau keterangan..." /></label>
      <label className="filter-label">Kategori<select value={category} onChange={(e) => setCategory(e.target.value)}><option>Semua</option>{categories.map((c) => <option key={c}>{c}</option>)}</select></label>
      ...
      <tbody>{loading ? <tr><td colSpan={6} className="table-empty">Memuat arsip...</td></tr> : filtered.length === 0 ? <tr><td colSpan={6} className="table-empty">Belum ada dokumen yang sesuai.</td></tr> : filtered.map((d, i) => <tr key={d.id}>
        <td>{i + 1}</td><td><strong>{d.title}</strong><small>{d.referenceNumber || "Tanpa nomor referensi"}</small></td><td><span className="category-pill">{d.category}</span></td><td><strong>{d.documentDate}</strong><small>Arsip {d.archiveYear}</small></td>
        ...
        <td><a className="download-button" href={`/api/documents/${d.id}`}>↓ Unduh</a></td>
      </tr>)}</tbody>
    </table></div><div className="table-footer">Menampilkan {filtered.length} dari {documents.length} dokumen</div></section>
