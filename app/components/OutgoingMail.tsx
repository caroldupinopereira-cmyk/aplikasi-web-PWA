...
export default function OutgoingMail({ initialOpenCreate = false }: { initialOpenCreate?: boolean }) {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      if (editing) {
        ...
        addToast("Surat keluar berhasil diperbarui.", "success");
      } else {
        const response = await fetch("/api/outgoing-letters", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        ...
        addToast("Surat keluar berhasil disimpan sebagai draf.", "success");
      }
      setForm(blankForm);
      setEditing(null);
      setShowForm(false);
  ...
  async function remove(id: number) {
    if (!confirm("Hapus surat keluar ini? Tindakan ini tidak dapat dibatalkan.")) return;
    try {
      const response = await fetch("/api/outgoing-letters", {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
      ...
      addToast("Surat keluar berhasil dihapus.", "success");
    } catch (error) {
    ...
    <div className="page mail-page outgoing-page">
      ...
      <section className="panel mail-table-panel">
        <div className="mail-toolbar">
          <label className="mail-search"><span>⌕</span><input value={query} onChange={(e) => { setQuery(e.target.value); setPage(1); }} placeholder="Cari nomor, tujuan, atau perihal..." /></label>
          ...
          <button className="export-button" onClick={() => exportToCSV(filtered.map((l, i) => ({ No: i + 1, "Nomor Surat": l.letterNumber, Tanggal: l.letterDate, Tujuan: l.recipient, Perihal: l.subject, Kategori: l.category, Penandatangan: l.signatory, Pengiriman: l.deliveryMethod, Status: l.status, Catatan: l.notes })), "surat_keluar")}>↓ Export CSV</button>
        </div>

        <div className="table-wrap">
          <table className="mail-table outgoing-table">
            <thead><tr><th>No.</th><th>Nomor & Tanggal</th><th>Tujuan & Perihal</th><th>Penandatangan</th><th>Pengiriman</th><th>Status</th><th>Aksi</th></tr></thead>
            <tbody>
              {loading ? (
                <tr><td colSpan={7} className="table-empty">Memuat data...</td></tr>
              ) : filtered.length === 0 ? (
                <tr><td colSpan={7} className="table-empty">Belum ada surat keluar yang sesuai.</td></tr>
        ...
        <div className="table-footer">Menampilkan {paged.length} dari {filtered.length} surat keluar</div>
        ...
        <div className="modal-layer" role="presentation" onMouseDown={(e) => e.target === e.currentTarget && setShowForm(false)}>
          <section className="mail-modal" role="dialog" aria-modal="true" aria-labelledby="outgoing-form-title">
            <div className="modal-heading">
              <div><p className="eyebrow">FORMULIR SURAT KELUAR</p><h2 id="outgoing-form-title">{editing ? "Edit Surat Keluar" : "Buat Surat Keluar"}</h2></div>
            ...
            <form onSubmit={submit}>
              ...
              <div className="form-actions"><button type="button" onClick={() => setShowForm(false)}>Batal</button><button className="save-button" disabled={saving}>{saving ? "Menyimpan..." : editing ? "Perbarui Surat" : "Simpan sebagai Draf"}</button></div>
            </form>
          </section>
        </div>
      )}
